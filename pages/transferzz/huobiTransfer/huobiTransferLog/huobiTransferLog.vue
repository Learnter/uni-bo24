<template>
	<view>
		...
		<view class="recoeBox">
      <navigator class="tradparLi uni-flex" v-for="(lItem,lIndex) in logsList" :key="lIndex" :url="'/pages/transferzz/huobiTransfer/huobiTransferLog/hbtlogDetails?logId='+lItem.id">
...
<script>
    ...
    onLoad() {
      uni.setNavigationBarTitle({
        title:this.$t('navText').h_t_record
      })
      this.fetchLogs();
    },
    onReachBottom(){ //上拉加载
    ...
    onPullDownRefresh(){ //下拉刷新
      ...
      fetchLogs(){ //获取日志列表
        ...
        this.$request.get(this.$KPI.transfer.hbyLogs,this.logConfig).then( res => {
           if(res.code === 200){
