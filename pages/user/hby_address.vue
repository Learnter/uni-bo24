	...
	<view class="shuruknzdBox">
		<form @submit="formSubmit" >		
			<view class="uni-form-item uni-column qxloninputBox">
				<view class="qxloninptl">{{i18n.hby_address}}：</view>
				<view class="with-fun qxloninput">
					<input type="text" class="uni-input" v-model="hbyInfo.hby_address" :placeholder="holder.hby_address" placeholder-style="color:#C4D8D8;" />				
				</view>
			</view>
			<view class="uni-form-item uni-column qxloninputBox">
				<view class="qxloninptl">{{i18n.tradePwd}}：</view>
				<view class="with-fun qxloninput">
					<input :password="showPassword" class="uni-input" v-model="hbyInfo.secpwd" :placeholder="holder.dealPwd" placeholder-style="color:#C4D8D8;" />				
          <view class="uni-icon uni-icon-eye" :class="[!showPassword ? 'uni-active' : '']" @click="changePassword"></view>
			...
			<view class="uni-form-item uni-column qxloninputBox">
				...
				<view class="with-fun qxloninput">
					<input class="uni-input" v-model="hbyInfo.mobileCode" :placeholder="holder.securityCode" placeholder-style="color:#C4D8D8;" />
...
<script>
    ...
    onLoad(){
      uni.setNavigationBarTitle({
        title:this.$t('navText').a_manage_hby
      })
    },
    onReady() {
      this.fetchUsdtInfo();
		...
		methods: {		
			fetchUsdtInfo(){
        this.$request.get(this.$KPI.USDT.convert,{type:5}).then(res => {
          if(res.code === 200){
            if(res.data && Object.keys(res.data).length !== 0){
              this.userPhone = res.data.mobile;
              this.hbyInfo.hby_address = res.data.hby_address;
            }
          }else{
            uni.showToast({
              icon:'none',
        ...
        //    return false; 
        //  }
        this.$request.post(this.$KPI.USDT.b_hby,this.hbyInfo).then(res => {
           if(res.code === 200){
             uni.showToast({
               icon:'loading',
