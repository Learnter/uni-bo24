	...
	<view>
		...
		<view class="recoeBox">
			<view v-for="(lItem,lIndex) in moneyLogs" :key="lIndex">
				<navigator class="tradparLi uni-flex" :url="'/pages/money/walletDetails?logId='+lItem.id">
...
<script>
    ...
    computed:{
      ...
      uni.setNavigationBarTitle({
        title:this.$t('navText').wallet
      })
			this.reConfig.mid = option.parentId;
		},
		onReady() {
			this.fetchType();
			this.fetchLogs();
		...
		methods: {
			fetchType() { //获取钱包分类
				this.$request.get(this.$KPI.moneyManage.moneyType).then(res => {
					if (res.code === 200){
        ...
        this.showEmpty = false;
				this.$request.get(this.$KPI.moneyManage.moneyLogs,this.reConfig).then(res => {
					if (res.code === 200) {
						if ( res.data.logList &&  Object.keys(res.data.logList).length !== 0) {
                this.moneyLogs = res.data.logList;
                this.reConfig.page++;
						} else {
							  this.showEmpty = true;
						}
        ...
        if(this.stopPull == true){
          ...
          this.$request.get(this.$KPI.moneyManage.moneyLogs, this.reConfig).then(res => {
          	if (res.code === 200 && res.data.logList) {
                ...
                this.stopPull = true
          			...
          			this.moneyLogs = this.moneyLogs.concat(res.data.logList);
          			this.reConfig.page++;
          		}
          	} else {
          		uni.showToast({
