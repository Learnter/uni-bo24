...
<script>
    ...
    computed:{
      i18n(){
        return this.$t('hbyExchange');
      }
    },
    onLoad(){
      uni.setNavigationBarTitle({
        title:this.$t('navText').hby_ec_huozhong
      })
    },
    onReady(){
      this.fetchExInfo();
      ...
      fetchExInfo(){ //获取兑换信息
        this.$request.get(this.$KPI.exchange.exHbyInfo).then(res => {
           // console.log(res);
      ...
      formSubmit(){ //提交兑换
       this.$request.post(this.$KPI.exchange.exHbyAction,this.exCommit).then( res => {
         if(res.code === 200){
           uni.showToast({
             icon:"loading",
	...
	onNavigationBarButtonTap(e) {// 转账日志
		uni.navigateTo({
			url:'/pages/exchange/hbyExchange/hbyExList'
		});
	}
}
</script>
