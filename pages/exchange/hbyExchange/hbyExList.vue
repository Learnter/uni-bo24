<template>
    ...
    <view  v-for="(lItem,lIndex) in logsList" :key="lIndex">
      <navigator class="tradparLi uni-flex"  :url="'/pages/exchange/hbyExchange/exhlDetails?logId='+lItem.id"  hover-class="none">
 ...
 export default {   
   ...
   onLoad(){
     uni.setNavigationBarTitle({
       title:this.$t('navText').hby_ec_log
     })
   },
   onReady() {
     this.fetchLogs();
   ...
   methods:{
     fetchLogs(){
       ...
       this.$request.get(this.$KPI.exchange.exHbyLogs,this.reConfig).then(res => {
         // console.log(res);
          if(res.code === 200  ){
