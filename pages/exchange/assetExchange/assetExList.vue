<template>
    ...
    <view v-for="(lItem,lIndex) in logsList" :key="lIndex">
      <navigator class="tradparLi uni-flex"  :url="'/pages/exchange/assetExchange/exlaDetails?logId='+lItem.id" hover-class="none">
 ...
 export default {   
   ...
   onLoad(){
      uni.setNavigationBarTitle({
        title:this.$t('navText').as_ec_log
      })
   },
   onReady(){
     this.fetchLogs();
   ...
   methods:{
     fetchLogs(){
       ...
       this.$request.get(this.$KPI.exchange.exAssetLogs,this.reConfig).then(res => {
          if(res.code === 200 ){
