...
<script>
    ...
    onReady(){
     let caches  = this.$catchs.get('offline_kwd');
      if(caches){
      ...
      /*存储搜索关键字,跳转店铺列表*/
        ...
        this.$catchs.put('offline_kwd', this.kwdCache);
        
        setTimeout(()=>{
          uni.navigateTo({
            url:"/pages/OfflineStore/storeList/storeList?kwd="+this.kwd
          })
        },500)
			},
      /*存储搜索关键字,跳转店铺列表*/
      ...
      delSearchLog(id){
      	uni.showModal({
      		...
      		success: (res) => {
      			if (res.confirm) {
      				this.$catchs.remove('offline_kwd');
      				this.kwdCache = [];
      			}
      		}
      	})
