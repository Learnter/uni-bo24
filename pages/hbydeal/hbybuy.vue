	...
	<view class="apbindaBox">
		<view class="apbindaCont">
			<form @submit="formSubmit">
                ...
                <mrhardSwitch class="switchClass" bgColorActive="#327cc0" bgColorDefault="#327cc0" textColor="#bfffc2" textFalse="购买" textTrue="出售" width="180rpx" height="60rpx" buttonSize='60rpx' textSize="32rpx" @change="changeSwitch"></mrhardSwitch>
				<view class="uni-form-item uni-column bindiiutBox">
					<view>买入数量</view>
					<view class="with-fun bindiiut">
						<input v-model="reConfig.buy_num" class="uni-input" type="number" placeholder="输入买入数量" placeholder-style="color:#888888;" />
					</view>
				</view>
				<view class="uni-form-item uni-column bindiiutBox">
					<view>交易密码</view>
...
<script>
  import mrhardSwitch from "@/components/mrhard-switch/mrhard-switch.vue";
    ...
    onLoad(){
      ...
      formSubmit(){ //提交操作
        let {buy_num,secpwd,pay_info} = this.reConfig;
        if(buy_num.trim().length === 0){
          uni.showToast({
            icon:'none',
            title:'请输入购买数量'
          })
        ...
        this.$request.post(this.$KPI.HbyDeal.buy,this.reConfig).then(res => {
          if(res.code === 200){
             uni.showToast({
               icon:"loading",
               title:'购买成功'
