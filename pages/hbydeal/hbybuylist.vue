<template>
	<view>
        ...
        <block v-for="(tItem,tIndex) in lItem.trade_list" :key="tIndex">
          <navigator :url="'/pages/hbydeal/hbybuydata?parent_id='+tItem.id" class="bvbnbnvzi"> 
...
<script>
  import uniCountdown from '@/components/uni-countdown/uni-countdown.vue'
    ...
    computed:{
      i18n(){
        return this.$t('hbybuylist')
      }
    },
		onLoad() {
      uni.setNavigationBarTitle({
        title:this.$t('navText').hBuyRecord
      })
			this.fetchLogs();
		},
		onReachBottom() { //上拉加载
      ...
      this.pullRefresh();
		...
		methods: {
			fetchLogs() {
				...
				this.$request.get(this.$KPI.HbyDeal.buyList, this.reConfig).then(res => {
					// console.log(res);
